<div class="containerTitles">
    <div class="containerTitles--title">
        <div class="containerTitles--title--left">Основное меню сайта</div>
        <div class="containerTitles--title--right"><a href="/panel.php?module=settings&action=9&add=1" class="button buttonSmall buttonGreen">Добавить новую ссылку</a></div>
    </div>
    <div class="containerTitles--content">
        ...
        foreach($items as $item){
            ...
            if($item['islink'] == 'y') $item['title'] = "<u>$item[title]</u>";
            ?>
            <tr class="contentTable--tr <?=$l_1?> <?=$l_2?> <?=$l_3?> <?=$l_4?> <?=$l_5?> <?=$status?>">
                <td align="center" nowrap=""><b><?=$item['id']?></b></td>
                <td class="title"><i class="flaticon-pointer level--url"></i><a href="/panel.php<?=$edit_link?>"><?=$item['title']?></a><a href="<?=$item['url']?>" target="_blank"><i class="flaticon-link link--url"></i></a></td>
                <td><?=$item['url']?></td>
                <td>
